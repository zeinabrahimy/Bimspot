...
namespace BimspotTest.UnitTest
{
	public class GetCriticalEndangered
	{
		...
		public async Task GetCriticalEndangered_InavalidRegions_ReturnsError(Service.Proxy.DTO.GetRegionResponse regions, string message, BimspotTestErrorResult errorResult)
		{
			// Arrange
		...
		public async Task GetCriticalEndangered_InvalidSpecies_ReturnsError(Service.Proxy.DTO.GetSpeciesOfRegionResponse species, string message)
		{
			// Arrange
			var iUCNProxy = new Moq.Mock<Service.Proxy.IIUCNProxy>();
			var speciesService = new BimspotTest.Service.SpeciesService(iUCNProxy.Object);
		...
		[Fact]
		public async Task GetCriterialEndangered_ThereIsNoCRInSpecies_ReturnsEmptyList()
		{
			// Arrange
			var iUCNProxy = new Moq.Mock<Service.Proxy.IIUCNProxy>();
			var speciesService = new BimspotTest.Service.SpeciesService(iUCNProxy.Object);
		...
		[Fact]
		public async Task GetCriticalEndangered_ThereIsOneCRInSpecies_ReturnsOneSpecies()
		{
			// Arrange
			var iUCNProxy = new Moq.Mock<Service.Proxy.IIUCNProxy>();
			var speciesService = new BimspotTest.Service.SpeciesService(iUCNProxy.Object);
			...
			var speciesResponse = new Service.Proxy.DTO.GetSpeciesOfRegionResponse
			{
				...
				RegionIdentifier = region.Identifier,
				Count = 1,
				Result = new List<Service.Proxy.DTO.Species>() { species }
			};
			...
			Assert.Equal(species.Category, returnedSpecies.Category);
			Assert.Equal(species.Population, returnedSpecies.Population);
			Assert.Equal(species.InfraName, returnedSpecies.InfraName);
			Assert.Equal(species.InfraRank, returnedSpecies.InfraRank);
			Assert.Equal(species.ScientificName, returnedSpecies.ScientificName);
			Assert.Equal(species.GenusName, returnedSpecies.GenusName);
			Assert.Equal(species.FamilyName, returnedSpecies.FamilyName);
			Assert.Equal(species.OrderName, returnedSpecies.OrderName);
			Assert.Equal(species.ClassName, returnedSpecies.ClassName);
			Assert.Equal(species.PhylumName, returnedSpecies.PhylumName);
			Assert.Equal(species.KingdomName, returnedSpecies.KingdomName);
			Assert.Equal(species.TaxonId, returnedSpecies.TaxonId);
			...
			Assert.Equal(speciesTaxonId, getSpeciesConservationMeasuresRequest);
		}

		[Fact]
		public async Task GetCriticalEndangered_ThereIsTwoCRInSpecies_ReturnTwoSpecieses()
		{
